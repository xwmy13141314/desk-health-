# DeskHealth 技术设计文档

> 版本：v1.0
> 日期：2025-01-19
> 设计师：Claude AI

---

## 1. 技术选型

### 1.1 最终方案：C# (.NET 8 WPF)

| 对比项 | C# (.NET 8 WPF) | Python + Tkinter |
|--------|-----------------|------------------|
| exe体积 | < 5MB | > 50MB |
| 内存占用 | < 20MB | 50-100MB |
| 启动速度 | < 1秒 | 2-3秒 |
| Windows体验 | 原生 | 一般 |
| 开发效率 | 中 | 高 |
| **推荐度** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |

**选择理由：**
- 完美符合PRD性能指标（CPU≤5%, 内存≤50MB）
- 原生Windows体验，系统集成更好
- 打包体积极小，便于分发

### 1.2 核心依赖

```xml
<PackageReference Include="Hardcodet.Wpf.TaskbarNotification" Version="1.0.5" />
<PackageReference Include="Microsoft.Extensions.DependencyInjection" Version="8.0.0" />
```

---

## 2. 系统架构

### 2.1 整体架构图

```
┌─────────────────────────────────────────────────────────────────┐
│                        DeskHealth 架构                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │                    Presentation Layer                      │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │ │
│  │  │  MainWindow  │  │ NotifyWindow │  │ TrayIconView │     │ │
│  │  │  (隐藏主窗口) │  │ (半透明提示) │  │  (托盘菜单)   │     │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘     │ │
│  └───────────────────────────────────────────────────────────┘ │
│                              ↕                                  │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │                   Application Layer                        │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │ │
│  │  │TimerService  │  │NotifyService │  │ConfigService │     │ │
│  │  │(计时器管理)  │  │(提醒显示)    │  │(配置管理)    │     │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘     │ │
│  └───────────────────────────────────────────────────────────┘ │
│                              ↕                                  │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │                     Domain Layer                           │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │ │
│  │  │  Reminder    │  │  TimerState  │  │  AppConfig  │     │ │
│  │  │  (提醒实体)  │  │  (计时状态)  │  │  (配置实体)  │     │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘     │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 项目结构

```
DeskHealth/
├── src/
│   ├── DeskHealth.Core/              # 领域层
│   │   ├── Entities/
│   │   │   ├── Reminder.cs           # 提醒实体
│   │   │   ├── TimerState.cs         # 计时状态
│   │   │   └── AppConfig.cs          # 配置实体
│   │   └── Interfaces/
│   │       ├── ITimerService.cs
│   │       ├── INotifyService.cs
│   │       └── IConfigService.cs
│   │
│   ├── DeskHealth.Services/          # 应用服务层
│   │   ├── TimerService.cs           # 计时器服务
│   │   ├── NotifyService.cs          # 提醒服务
│   │   └── ConfigService.cs          # 配置服务
│   │
│   └── DeskHealth.App/               # 表示层（WPF）
│       ├── Views/
│       │   ├── MainWindow.xaml       # 主窗口（隐藏）
│       │   ├── NotifyWindow.xaml     # 半透明提醒窗
│       │   └── AboutWindow.xaml      # 关于对话框
│       ├── ViewModels/
│       │   └── NotifyViewModel.cs    # 提醒窗ViewModel
│       ├── Services/
│       │   └── TrayIconService.cs    # 托盘服务
│       ├── App.xaml
│       └── App.xaml.cs
│
├── docs/
│   ├── 技术设计文档.md
│   └── API文档.md
│
├── tests/
│   └── DeskHealth.Tests/
│
├── DeskHealth.sln
└── README.md
```

---

## 3. 核心模块设计

### 3.1 TimerService（计时器服务）

**职责：**
- 管理多个提醒计时器（喝水、休息）
- 处理暂停/恢复逻辑
- 触发提醒事件

**状态机：**

```
                    ┌──────────────────┐
                    │     RUNNING      │ ◄── 启动默认状态
                    │   (正常运行)      │
                    └────────┬─────────┘
                             │
              ┌──────────────┼──────────────┐
              │              │              │
         暂停1小时       暂停2小时          │
              │              │              │
              ▼              ▼              │
       ┌────────────┐  ┌────────────┐      │
       │ PAUSED_1H  │  │ PAUSED_2H  │      │
       └────────────┘  └────────────┘      │
              │              │              │
              └──────────────┴──────────────┘
                             │
                        恢复/超时结束
                             │
                             ▼
                    ┌──────────────────┐
                    │     RUNNING      │
                    └──────────────────┘
```

**接口定义：**

```csharp
public interface ITimerService
{
    event EventHandler<ReminderEventArgs> ReminderTriggered;
    TimerState State { get; }
    void Start();
    void Pause(TimeSpan duration);
    void Resume();
    void Stop();
}

public class TimerService : ITimerService, IDisposable
{
    private readonly Dictionary<ReminderType, Timer> _timers;
    private readonly IConfigService _configService;
    private TimerState _state;
    private CancellationTokenSource _pauseCts;

    public event EventHandler<ReminderEventArgs> ReminderTriggered;

    public TimerState State => _state;

    public void Start()
    {
        _state = TimerState.Running;
        InitializeTimers();
    }

    public void Pause(TimeSpan duration)
    {
        _state = duration.TotalHours >= 2
            ? TimerState.Paused2H
            : TimerState.Paused1H;

        _pauseCts?.Cancel();
        _pauseCts = new CancellationTokenSource();

        Task.Delay(duration, _pauseCts.Token)
            .ContinueWith(_ => Resume());
    }

    public void Resume()
    {
        _state = TimerState.Running;
        _pauseCts?.Cancel();
    }

    private void InitializeTimers()
    {
        // 喝水提醒：30分钟
        _timers[ReminderType.Hydration] = new Timer(
            _ => OnReminderTriggered(ReminderType.Hydration),
            null,
            TimeSpan.FromMinutes(30),
            TimeSpan.FromMinutes(30));

        // 休息提醒：60分钟
        _timers[ReminderType.Break] = new Timer(
            _ => OnReminderTriggered(ReminderType.Break),
            null,
            TimeSpan.FromMinutes(60),
            TimeSpan.FromMinutes(60));
    }

    private void OnReminderTriggered(ReminderType type)
    {
        if (_state != TimerState.Running) return;

        var reminder = _configService.GetReminder(type);
        ReminderTriggered?.Invoke(this, new ReminderEventArgs(reminder));
    }

    public void Dispose()
    {
        foreach (var timer in _timers.Values)
        {
            timer.Dispose();
        }
        _pauseCts?.Cancel();
        _pauseCts?.Dispose();
    }
}
```

### 3.2 NotifyService（提醒服务）

**职责：**
- 显示/隐藏半透明提醒窗口
- 管理自动关闭倒计时
- 处理用户手动关闭

**接口定义：**

```csharp
public interface INotifyService
{
    void ShowReminder(Reminder reminder);
    void HideReminder();
}

public class NotifyService : INotifyService
{
    private readonly NotifyWindow _window;
    private Timer _autoCloseTimer;

    public NotifyService()
    {
        _window = new NotifyWindow();
        _window.Closed += OnWindowClosed;
    }

    public void ShowReminder(Reminder reminder)
    {
        _window.DataContext = reminder;

        // 计算屏幕右下角位置
        var screenWidth = SystemParameters.PrimaryScreenWidth;
        var screenHeight = SystemParameters.PrimaryScreenHeight;
        var taskbarHeight = GetTaskbarHeight();

        _window.Left = screenWidth - _window.Width - 10;
        _window.Top = screenHeight - taskbarHeight - _window.Height - 10;

        // 关键：不激活窗口，不抢焦点
        _window.ShowActivated = false;
        _window.Show();

        // 10秒后自动关闭
        _autoCloseTimer?.Dispose();
        _autoCloseTimer = new Timer(
            _ => HideReminder(),
            null,
            TimeSpan.FromSeconds(10),
            Timeout.InfiniteTimeSpan);
    }

    public void HideReminder()
    {
        _window.Dispatcher.Invoke(() =>
        {
            _window.Hide();
            _autoCloseTimer?.Dispose();
        });
    }

    private double GetTaskbarHeight()
    {
        // 检测任务栏位置并返回相应高度/宽度
        var workingArea = SystemParameters.WorkArea;
        if (workingArea.Bottom < SystemParameters.PrimaryScreenHeight)
            return SystemParameters.PrimaryScreenHeight - workingArea.Bottom;
        return 40; // 默认值
    }
}
```

### 3.3 ConfigService（配置服务）

**职责：**
- 管理应用配置
- 持久化到JSON文件
- 提供默认配置

**配置文件路径：** `%AppData%/DeskHealth/config.json`

```csharp
public interface IConfigService
{
    AppConfig LoadConfig();
    void SaveConfig(AppConfig config);
    Reminder GetReminder(ReminderType type);
}

public class ConfigService : IConfigService
{
    private readonly string _configPath;
    private AppConfig _cachedConfig;

    public ConfigService()
    {
        var appDataPath = Environment.GetFolderPath(
            Environment.SpecialFolder.ApplicationData);
        var appFolder = Path.Combine(appDataPath, "DeskHealth");

        Directory.CreateDirectory(appFolder);
        _configPath = Path.Combine(appFolder, "config.json");
    }

    public AppConfig LoadConfig()
    {
        if (_cachedConfig != null)
            return _cachedConfig;

        if (!File.Exists(_configPath))
        {
            _cachedConfig = GetDefaultConfig();
            SaveConfig(_cachedConfig);
            return _cachedConfig;
        }

        var json = File.ReadAllText(_configPath);
        _cachedConfig = JsonSerializer.Deserialize<AppConfig>(json);
        return _cachedConfig;
    }

    public void SaveConfig(AppConfig config)
    {
        var json = JsonSerializer.Serialize(config, new JsonSerializerOptions
        {
            WriteIndented = true
        });
        File.WriteAllText(_configPath, json);
        _cachedConfig = config;
    }

    public Reminder GetReminder(ReminderType type)
    {
        var config = LoadConfig();
        return config.Reminders.FirstOrDefault(r => r.Type == type)
            ?? throw new ArgumentException($"No reminder found for type: {type}");
    }

    private static AppConfig GetDefaultConfig()
    {
        return new AppConfig
        {
            Version = "1.0.0",
            AutoStart = true,
            EnableSound = false, // MVP默认静音
            Reminders = new List<Reminder>
            {
                new Reminder
                {
                    Type = ReminderType.Hydration,
                    Title = "该喝水了 🥤",
                    Message = "保持健康，多喝水！",
                    IntervalMinutes = 30,
                    AutoCloseSeconds = 10
                },
                new Reminder
                {
                    Type = ReminderType.Break,
                    Title = "休息一下 🌿",
                    Message = "站起来活动活动，保护颈椎！",
                    IntervalMinutes = 60,
                    AutoCloseSeconds = 10
                }
            }
        };
    }
}
```

---

## 4. UI/UX 设计

### 4.1 提醒窗口

**视觉效果：**

```
┌─────────────────────────────────────────┐
│  该喝水了 🥤                           [×] │  ← 200px × 80px
│                                         │
│  保持健康，多喝水！                      │
└─────────────────────────────────────────┘
   ↑ 右下角位置（贴合任务栏上方）
```

**XAML 实现：**

```xml
<!-- NotifyWindow.xaml -->
<Window x:Class="DeskHealth.App.Views.NotifyWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Width="200" Height="80"
        WindowStyle="None"
        AllowsTransparency="True"
        Background="Transparent"
        Topmost="True"
        ShowActivated="False"
        ShowInTaskbar="False"
        ResizeMode="NoResize">

    <Border Background="#E6E6E6"
            Opacity="0.7"
            CornerRadius="8"
            BorderBrush="#CCCCCC"
            BorderThickness="1">
        <Grid>
            <Grid.RowDefinitions>
                <RowDefinition Height="Auto"/>
                <RowDefinition Height="*"/>
            </Grid.RowDefinitions>

            <!-- 关闭按钮 -->
            <Button Grid.Row="0"
                    Content="×"
                    Width="24"
                    Height="24"
                    HorizontalAlignment="Right"
                    VerticalAlignment="Top"
                    Margin="0,4,8,0"
                    Background="Transparent"
                    BorderThickness="0"
                    FontSize="16"
                    Foreground="#666666"
                    Cursor="Hand"
                    Click="OnCloseClicked">
                <Button.Style>
                    <Style TargetType="Button">
                        <Style.Triggers>
                            <Trigger Property="IsMouseOver" Value="True">
                                <Setter Property="Foreground" Value="#FF0000"/>
                            </Trigger>
                        </Style.Triggers>
                    </Style>
                </Button.Style>
            </Button>

            <!-- 提醒内容 -->
            <StackPanel Grid.Row="1"
                        VerticalAlignment="Center"
                        Margin="20,8">
                <TextBlock Text="{Binding Title}"
                          FontSize="14"
                          FontWeight="SemiBold"
                          Foreground="#333333"/>
                <TextBlock Text="{Binding Message}"
                          FontSize="11"
                          Foreground="#555555"
                          Margin="0,4,0,0"
                          TextWrapping="Wrap"/>
            </StackPanel>
        </Grid>
    </Border>
</Window>
```

**关键属性说明：**

| 属性 | 值 | 作用 |
|------|-----|------|
| `WindowStyle` | None | 无边框 |
| `AllowsTransparency` | True | 允许透明 |
| `Background` | Transparent | 窗口背景透明 |
| `ShowActivated` | False | 不激活窗口（不抢焦点） |
| `Topmost` | True | 置顶显示 |
| `ShowInTaskbar` | False | 不显示在任务栏 |

### 4.2 系统托盘

**菜单结构：**

```
┌─────────────────────────┐
│ 💧 DeskHealth v1.0.0   │
├─────────────────────────┤
│ 关于 DeskHealth...      │
├─────────────────────────┤
│ 暂停提醒 1 小时         │
│ 暂停提醒 2 小时         │
│ 恢复提醒                │ ← 仅暂停时显示
├─────────────────────────┤
│ 退出                    │
└─────────────────────────┘
```

**实现代码：**

```csharp
public class TrayIconService
{
    private readonly NotifyIcon _notifyIcon;
    private readonly ITimerService _timerService;

    public TrayIconService(ITimerService timerService)
    {
        _timerService = timerService;
        _notifyIcon = new NotifyIcon
        {
            Icon = Properties.Resources.AppIcon,
            Text = "DeskHealth - 桌面健康提醒",
            Visible = true
        };

        SetupContextMenu();
    }

    private void SetupContextMenu()
    {
        var menu = new ContextMenuStrip();

        // 版本信息（不可点击）
        var titleItem = new ToolStripLabel($"DeskHealth v{GetVersion()}")
        {
            Font = new Font("Segoe UI", 9, FontStyle.Bold)
        };
        menu.Items.Add(new ToolStripControlHost(titleItem));
        menu.Items.Add(new ToolStripSeparator());

        // 关于
        menu.Items.Add("关于 DeskHealth...", null, OnAboutClicked);

        // 暂停/恢复
        menu.Items.Add(new ToolStripSeparator());
        menu.Items.Add("暂停提醒 1 小时", null, _ => Pause(1));
        menu.Items.Add("暂停提醒 2 小时", null, _ => Pause(2));

        var resumeItem = new ToolStripMenuItem("恢复提醒", null, _ => Resume());
        resumeItem.Name = "ResumeItem";
        menu.Items.Add(resumeItem);

        // 退出
        menu.Items.Add(new ToolStripSeparator());
        menu.Items.Add("退出", null, OnExitClicked);

        _notifyIcon.ContextMenuStrip = menu;
        UpdateMenuState();
    }

    private void Pause(int hours)
    {
        _timerService.Pause(TimeSpan.FromHours(hours));
        UpdateMenuState();
    }

    private void Resume()
    {
        _timerService.Resume();
        UpdateMenuState();
    }

    private void UpdateMenuState()
    {
        var menu = _notifyIcon.ContextMenuStrip;
        var resumeItem = menu.Items.Find("ResumeItem", false).FirstOrDefault() as ToolStripMenuItem;

        if (_timerService.State == TimerState.Running)
        {
            resumeItem.Visible = false;
        }
        else
        {
            resumeItem.Visible = true;
            resumeItem.Text = _timerService.State == TimerState.Paused1H
                ? "恢复提醒 (剩余 XX 分钟)"
                : "恢复提醒 (剩余 XX 分钟)";
        }
    }

    public void Dispose()
    {
        _notifyIcon?.Dispose();
    }
}
```

---

## 5. 领域模型

### 5.1 实体定义

```csharp
// 提醒类型
public enum ReminderType
{
    Hydration,  // 喝水
    Break       // 休息
}

// 计时状态
public enum TimerState
{
    Running,
    Paused1H,
    Paused2H
}

// 提醒实体
public class Reminder
{
    public ReminderType Type { get; set; }
    public string Title { get; set; }
    public string Message { get; set; }
    public int IntervalMinutes { get; set; }
    public int AutoCloseSeconds { get; set; }
}

// 应用配置
public class AppConfig
{
    public string Version { get; set; }
    public bool AutoStart { get; set; }
    public bool EnableSound { get; set; }
    public List<Reminder> Reminders { get; set; }
}
```

---

## 6. 开机自启动

### 6.1 实现方式

通过注册表 `Run` 键值实现：

```csharp
public static class AutoStartHelper
{
    private const string RunKeyPath = @"Software\Microsoft\Windows\CurrentVersion\Run";
    private const string AppName = "DeskHealth";

    public static void SetAutoStart(bool enable)
    {
        using var key = Registry.CurrentUser.OpenSubKey(RunKeyPath, true);
        var exePath = Process.GetCurrentProcess().MainModule?.FileName;

        if (enable)
        {
            key?.SetValue(AppName, $"\"{exePath}\"");
        }
        else
        {
            key?.DeleteValue(AppName, false);
        }
    }

    public static bool IsAutoStartEnabled()
    {
        using var key = Registry.CurrentUser.OpenSubKey(RunKeyPath, false);
        var value = key?.GetValue(AppName);
        return value != null;
    }
}
```

---

## 7. 单例运行

### 7.1 Mutex 实现

```csharp
public partial class App : Application
{
    private const string MutexName = "Global\\DeskHealth_SingleInstanceMutex";
    private Mutex _mutex;

    protected override void OnStartup(StartupEventArgs e)
    {
        // 检查是否已有实例运行
        _mutex = new Mutex(true, MutexName, out bool createdNew);

        if (!createdNew)
        {
            MessageBox.Show(
                "DeskHealth 已经在运行中。",
                "提示",
                MessageBoxButton.OK,
                MessageBoxImage.Information);
            Shutdown();
            return;
        }

        base.OnStartup(e);
    }

    protected override void OnExit(ExitEventArgs e)
    {
        _mutex?.ReleaseMutex();
        _mutex?.Dispose();
        base.OnExit(e);
    }
}
```

---

## 8. 性能优化

### 8.1 资源控制目标

| 指标 | 目标值 | 实现方式 |
|------|--------|----------|
| CPU占用 | ≤ 5% | 使用 `Timer` 而非 `DispatcherTimer` |
| 内存占用 | ≤ 50MB | 延迟加载、及时释放资源 |
| 启动时间 | ≤ 1秒 | 异步初始化、延迟加载非核心资源 |
| 运行稳定性 | 72h+ 无崩溃 | 异常处理 + 日志记录 |

### 8.2 优化措施

```csharp
// 1. 使用 Timer 而非 DispatcherTimer
// Timer 使用线程池，不占用 UI 线程
private Timer _timer;
_timer = new Timer(callback, null, dueTime, period);

// 2. 及时释放定时器
public void Dispose()
{
    _autoCloseTimer?.Dispose();
    _pauseCts?.Cancel();
    _pauseCts?.Dispose();
}

// 3. 延迟加载非核心资源
private void LoadNonCriticalResourcesAsync()
{
    Task.Run(() =>
    {
        // 后台加载图标、资源等
    });
}
```

---

## 9. 打包与分发

### 9.1 发布配置

```xml
<!-- 项目文件配置 -->
<PropertyGroup>
    <OutputType>WinExe</OutputType>
    <TargetFramework>net8.0-windows</TargetFramework>
    <UseWPF>true</UseWPF>
    <ImplicitUsings>enable</ImplicitUsings>
    <Nullable>enable</Nullable>
    <PublishSingleFile>true</PublishSingleFile>
    <SelfContained>true</SelfContained>
    <RuntimeIdentifier>win-x64</RuntimeIdentifier>
    <PublishTrimmed>true</PublishTrimmed>
    <IncludeNativeLibrariesForSelfExtract>true</IncludeNativeLibrariesForSelfExtract>
</PropertyGroup>
```

### 9.2 发布命令

```bash
# 发布为单个 exe
dotnet publish -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true

# 输出大小预计：< 10MB
```

### 9.3 分发方式

- **绿色版**：单文件 exe，直接运行
- **安装版**：使用 WiX Toolset 创建 MSI 安装包

---

## 10. 测试计划

### 10.1 功能测试

| 测试项 | 预期结果 |
|--------|----------|
| 启动应用 | 托盘图标显示，计时器开始 |
| 30分钟后 | 显示喝水提醒，10秒后自动消失 |
| 点击关闭按钮 | 提醒立即消失 |
| 暂停1小时 | 计时器暂停，1小时后恢复 |
| 恢复提醒 | 计时器继续运行 |
| 退出应用 | 托盘图标消失，程序关闭 |

### 10.2 性能测试

- 使用 `PerformanceMonitor` 监控 CPU/内存
- 连续运行 72 小时稳定性测试
- 高 DPI (200%, 300%) 显示测试

---

## 11. 下一步行动

1. **技术选型确认** ✅ (已完成，采用 C# .NET 8 WPF)
2. **项目初始化** (创建解决方案、配置依赖)
3. **核心功能开发**
   - [ ] TimerService 实现
   - [ ] NotifyWindow UI 实现
   - [ ] TrayIconService 实现
   - [ ] ConfigService 实现
4. **集成测试**
5. **小范围灰度测试** (2-3名同事试用1天)
6. **正式发布**

---

## 附录

### A. 参考资料

- [WPF 官方文档](https://docs.microsoft.com/en-us/dotnet/desktop/wpf/)
- [Hardcodet.NotifyIcon](https://github.com/hardcodet/wpf-notifyicon)
- [.NET 8 发布单文件应用](https://docs.microsoft.com/en-us/dotnet/core/deploying/single-file/)

### B. 版本历史

| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v1.0 | 2025-01-19 | 初始设计文档 |
